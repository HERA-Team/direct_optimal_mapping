...
class OptMapping:
    ...
    def __init__(self, uv, nside, epoch='J2000'):
        ...
        '''
        self.hera_site = EarthLocation(
            lat=uv.telescope_location_lat_lon_alt_degrees[0]*u.deg,
    ...
    def set_a_mat(self):
        ...
        for time_t in np.unique(self.uv.time_array):
            az_t, alt_t = self._radec2azalt(
                self.ra[self.idx_psf_in],
                self.dec[self.idx_psf_in],
                time_t,
            )
            lmn_t = np.array([
                np.cos(alt_t)*np.sin(az_t),
            ...
            for i in range(len(idx_time)):
                irow = idx_time[i]
                a_mat[irow] = +2*np.pi/self.wavelength*np.matmul(
                    np.matrix(self.uv.uvw_array[irow].astype(np.float32)),
    ...
    def set_a_mat_ps(self, ps_radec):
        ...
        a_mat = np.zeros(
            (
                len(self.data), len(self.idx_psf_in) +
                ps_radec.shape[0],
            ), dtype='float32',
        )
        beam_mat = np.zeros(a_mat.shape, dtype='float32')
        ...
        for time_t in np.unique(self.uv.time_array):
            az_t, alt_t = self._radec2azalt(
                np.concatenate((self.ra[self.idx_psf_in], ra_ps)),
                np.concatenate(
                    (self.dec[self.idx_psf_in], dec_ps),
                ),
                time_t,
            )
            lmn_t = np.array([
    ...
    def set_k_facet(self, radius_deg, calc_k=False):
        ...
        if calc_k:
            ...
            idx_facet_out_psf = np.where(
                (np.pi/2. - self.alt[self.idx_psf_in]) > facet_radius,
            )[0]
